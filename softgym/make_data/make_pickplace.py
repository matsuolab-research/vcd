...
class MakePickPlace:
    ...
    def process(self):
        ...
        for i in range(self.num_downpcd):
            for j in range(self.num_downpcd):
                if i == j:
                    continue
                if self.is_connected(i, j) or self.get_dist(i, j) < self.threshold_r:
                    num_edges += 1
                    edge_from.append(i)
                    edge_to.append(j)
        ...
        pass_w = './pickplace_data/pp' + ('000' + str(self.num))[-4:] + '.txt'
        ...
        with open(pass_w, 'w') as f:
            ...
            for i in range(60):
                action = self.env.action_space.sample()
                action[0] = random.uniform(-0.5, 0.5)
                action[1] = random.uniform(0.0, 0.5)
                action[2] = random.uniform(-0.5, 0.5)
                move_dist = random.uniform(0.15, 0.4)
        ...
        #     save_name = osp.join(self.save_video_dir, 'hogehoge.gif')
    ...
    def downsample_point_cloud(self):
        ...
        self.downpcd = pcd.voxel_down_sample(voxel_size=0.0216)
